...
#include "SkeletonDetection.h"
#include <fstream>

namespace SkeletonViewerApplication
{

	SkeletonDetection::SkeletonDetection(const SkelType& _type)
	{
		type = _type;
	...
	SkeletonDetection SkeletonDetection::Mapping(const SkelType& tarType)
	{
		SkeletonDetection detection(tarType);
	...
	std::vector<Eigen::Matrix3Xf> SkeletonDetection::Associate(const int& jcntThresh)
	{
	...
	std::vector<SkeletonDetection> ParseDetections(const std::string& filename)
	{
		...
		std::vector<SkeletonDetection> detections(frameSize, SkeletonDetection(SkelType(skelType)));
		for (SkeletonDetection& detection : detections) {
			for (int jIdx = 0; jIdx < def.jointSize; jIdx++) {
				int jSize;
				fs >> jSize;
			...
			for (int pafIdx = 0; pafIdx < def.pafSize; pafIdx++) {
				...
				for (int i = 0; i < detection.pafs[pafIdx].rows(); i++)
					for (int j = 0; j < detection.pafs[pafIdx].cols(); j++)
					{
						...
						//std::cout << " pafs value: " << detection.pafs[pafIdx](i, j) << std::endl;
					}
	...
	void SerializeDetections(const std::vector<SkeletonDetection>& detections, const std::string& filename)
	{
		...
		for (const SkeletonDetection& detection : detections) {
			for (int jIdx = 0; jIdx < def.jointSize; jIdx++)
